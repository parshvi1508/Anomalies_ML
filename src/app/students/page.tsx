        ...
        {Object.entries(riskCategoryCounts).map(([category, count]) => (
          <div key={category} className="bg-white rounded-lg shadow p-4 border">
            <div className="flex items-center gap-2">
              <span className="text-lg">{riskCategoryIcons[category as keyof typeof riskCategoryIcons] || '⚪'}</span>
              <div>
                <div className="text-2xl font-bold text-gray-900">{count}</div>
                <div className="text-sm text-gray-500 capitalize">{category} Risk</div>
              </div>
            </div>
          </div>
        ))}
        ...
        <div className="flex flex-col md:flex-row gap-4">
          ...
          <div>
            ...
            <select
              id="sort"
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as 'id' | 'riskCategory')}
